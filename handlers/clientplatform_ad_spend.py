...
def install_ad_spend_controls(
    ...
    """Add one advanced-dashboard entry without changing the simple first-run path."""
...
@simple.router.callback_query(F.data.startswith("cpsp:home:"))
...
@simple.router.callback_query(F.data.startswith("cpsp:job:"))
async def choose_ad_spend_job(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        business_id = control._token_uuid(business_token)
        publication_job_id = control._token_uuid(job_token)
...
@simple.router.message(AdSpendConsentState.waiting_hard_cap)
...
@simple.router.message(AdSpendConsentState.waiting_daily_cap)
async def receive_ad_spend_daily_cap(message: Message, state: FSMContext) -> None:
    ...
    await state.update_data(
        ...
        authorization_token=control._uuid_token(authorization.id),
    )
            ...
            [
                [
                    (
                        ...
                        "cpsp:confirm:"
                        f"{data['business_token']}:{control._uuid_token(authorization.id)}",
                    )
                ],
                [
                    (
...
async def confirm_ad_spend_consent(callback: CallbackQuery, state: FSMContext) -> None:
    try:
        ...
        business_id = control._token_uuid(business_token)
        authorization_id = control._token_uuid(authorization_token)
        actor = await control._actor(int(callback.from_user.id), business_id)
...
@simple.router.callback_query(F.data.startswith("cpsp:revoke:"))
