...
async def _workspace(callback: CallbackQuery, *, business_token: str) -> None:
    ...
    next_step = (
        "\n\nКабинет готов. Чтобы выбрать услугу и свободное время для рекламного "
        "черновика, нажмите «🎯 Создать рекламу»."
        if active
    ...
    await _message(callback).answer(
        ...
        "получает доступ к кабинетам других пользователей и не запускает показы "
        "или расходы автоматически.\n\n"
        "Подключения:\n"
