...
2. ClientPlatform извлекает только явно сообщённые факты и показывает понятные структурированные данные о бизнесе;
3. владелец подтверждает их через «Всё верно» или исправляет через «Изменить»;
4. ClientPlatform предлагает выбрать первый полезный результат без provider/API terminology;
5. активируется только необходимая для выбранного результата capability, а существующие booking/program/customer flows переиспользуются;
6. прерванный DRAFT onboarding корректно продолжается после `/start`/повторного входа без возврата в legacy setup.

### AI

AI может извлекать/предлагать структуру, но подтверждённый `BusinessProfile` — durable domain state, а не prompt memory. Неизвестные цены, аудитория, legal/qualification claims и conversion action не выдумываются.
...
- P1 restart/resume regression закрыт на composed production router: DRAFT продолжает U-007 review/first-result flow после interaction-safety wrapper, а не уходит в legacy capability setup.
- Production 2026-08-19 обновлён exact merge SHA `26ea24496ebcca37c5e6e0f04ac4814d5175d965`: fail-closed preflight и deploy завершились успешно, encrypted pre-deploy backup создан, internal health/readiness `ok=true`, public `/healthz`/`/readyz` и Telegram webhook-prefix остаются закрыты `404` при polling-only transport.
...
| U-007 Zero-to-First-Outcome Onboarding | DONE | PR #198; merge SHA `26ea24496ebcca37c5e6e0f04ac4814d5175d965`; all 15 required PR workflows success on `25de33dc42b5a97475bb12c306e925628d88d576`; coverage ratchets raised to 74.30% combined / 65.33% branch; exact merge SHA deployed to production with fail-closed preflight/deploy and healthy internal probes |
