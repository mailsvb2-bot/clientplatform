...
Не вставляйте в Tilda прямые `t.me`, `vk.com` или `max.ru` deep link. Runtime сам строит provider URL с owner payload `cpo_landing`. Это отделяет вход владельца от customer acquisition (`cpa_*`) и customer invite (`cpj_*`).
