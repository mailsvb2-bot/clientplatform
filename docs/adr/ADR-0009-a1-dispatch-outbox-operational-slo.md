# ADR-0009: DB-backed operational SLO для A1 dispatch outbox
...
ADR-0008 сделал lifecycle и scheduler A1 dispatch наблюдаемыми. Однако работающий scheduler ещё не доказывает, что очередь обслуживается с приемлемой скоростью. Возможны состояния:
...
Outbox probe выполняется только когда `A1_DISPATCH_RUNTIME_ENABLED=1`. Выключенный A1 не делает лишних запросов и не влияет на legacy readiness.

## Readiness thresholds

- `A1_DISPATCH_READY_MAX_DUE`, default `1000`;
- `A1_DISPATCH_READY_MAX_OLDEST_DUE_AGE_SEC`, default `900`;
- `A1_DISPATCH_READY_MAX_STALE_SENDING`, default `0`;
- `A1_DISPATCH_READY_MAX_RECENT_DEAD`, default `100`;
- `A1_DISPATCH_READY_DEAD_WINDOW_SEC`, default `900`.

Stale lease рассчитывается относительно канонического `A1_DISPATCH_LOCK_TTL_SEC`.

## Ошибки readiness

- `a1_dispatch_outbox:unavailable`;
- `a1_dispatch_outbox:due_backlog`;
- `a1_dispatch_outbox:oldest_due`;
- `a1_dispatch_outbox:stale_sending`;
- `a1_dispatch_outbox:recent_dead`.
...
- выключенный A1 не обращается к outbox;
...
- A1 Boundary Diagnostics запускает outbox observability regression wall.
