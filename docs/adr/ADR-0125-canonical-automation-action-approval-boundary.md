...
2. Approval intent содержит exact canonical `CandidateAction` JSON + SHA-256 fingerprint, business-scoped idempotency key, exact `AutomationPolicy` id/version/hash, причины `approval_required`, requester evidence и expiry. Для external write кандидат до создания intent обязан содержать opaque immutable `subject_ref` и exact SHA-256 `payload_digest`; оба входят в candidate fingerprint и показываются владельцу до решения.
...
7. Approved intent может породить только детерминированный internal `AutomationActionAuthorization`, привязанный к exact approval/candidate/policy/owner evidence. Для external write authorization дополнительно требует от будущего executor точного совпадения `subject_ref` и `payload_digest` с approved candidate. Это не bearer-token и не provider credential; он действителен только после повторной server-side проверки текущей policy.
