...
Proposed — 2026-07-29. This ADR becomes accepted only when the exact-head PostgreSQL, release, security and runtime workflows are green.
...
3. A business has at most one active managed Telegram bot in this foundation. Replacement requires disabling or revoking the previous route first.
...
11. Full payload JSON is removed after `processed` or `dead`; update ID, digest, timestamps and sanitized error code remain as replay/audit evidence.
...
13. Dispatcher workflow data carries `managed_bot_business_id`, `managed_bot_id` and `managed_bot_connection_id`. `/start` in a personal bot opens only that specialist's customer portal and never owner onboarding.
...
- Operational readiness now requires PostgreSQL two-connection evidence, queue/fleet health and tested restore of gateway tables.
