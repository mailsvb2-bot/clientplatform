...
def media_gateway_configured() -> bool:
    return _env_bool("CLIENTPLATFORM_MEDIA_GATEWAY_ENABLED", False)
...
def media_gateway_config() -> MediaGatewayConfig:
    return MediaGatewayConfig(
        ...
        host=str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_HOST") or "127.0.0.1").strip(),
        port=env_int(
        ...
        public_base_url=str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_BASE_URL") or "").strip().rstrip("/"),
        storage_mode=str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_STORAGE_MODE") or "").strip().lower(),
        allowed_buckets=_allowed_buckets(
            str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_ALLOWED_BUCKETS") or "")
        ),
        filesystem_root=str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_FILESYSTEM_ROOT") or "").strip(),
        s3_endpoint=str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_S3_ENDPOINT") or "").strip().rstrip("/"),
        s3_region=str(os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_S3_REGION") or "").strip(),
        s3_access_key_reference=str(
            os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_S3_ACCESS_KEY_REFERENCE")
            or "secret://env/CLIENTPLATFORM_SECRET_S3_ACCESS_KEY"
        ).strip(),
        s3_secret_key_reference=str(
            os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_S3_SECRET_KEY_REFERENCE")
            or "secret://env/CLIENTPLATFORM_SECRET_S3_SECRET_KEY"
        ).strip(),
        s3_session_token_reference=str(
            os.getenv("CLIENTPLATFORM_MEDIA_GATEWAY_S3_SESSION_TOKEN_REFERENCE") or ""
        ).strip(),
        signing_secret_reference=str(
...
def media_gateway_health_snapshot() -> dict[str, Any]:
    return {
        "clientplatform_media_gateway_configured": media_gateway_configured(),
        "clientplatform_media_gateway_health_available": True,
        "clientplatform_media_gateway_running": _health.running,
        "clientplatform_media_gateway_requests": _health.requests,
        "clientplatform_media_gateway_denied": _health.denied,
        "clientplatform_media_gateway_not_found": _health.not_found,
        "clientplatform_media_gateway_upstream_errors": _health.upstream_errors,
        "clientplatform_media_gateway_bytes_streamed": _health.bytes_streamed,
        "clientplatform_media_gateway_last_error": _health.last_error,
    }
...
async def start_media_gateway_runtime(
    ...
    *,
    ...
    _health.last_error = ""
    log.info("clientplatform media gateway started on %s:%s", selected.host, selected.port)
    return runtime
...
async def run_media_gateway_owner(
    *,
    ...
    try:
        ...
        log.info("clientplatform media gateway stopped")
