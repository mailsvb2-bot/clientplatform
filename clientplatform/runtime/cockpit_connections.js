  ...
  const render = (payload) => {
    ...
    for (const item of items) {
      ...
      text(note, item.active
        ? "Канал подключён к этому бизнесу и используется через общий runtime ClientPlatform."
        : item.can_connect
          ? "Можно подключить сейчас. Токен вводится только на одноразовой HTTPS-странице."
