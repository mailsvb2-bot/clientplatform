...
class ClientPlatformActiveBookingBoundaryTests(unittest.TestCase):
    def test_active_customer_booking_rejects_started_appointment(self) -> None:
        repository = SimpleNamespace(
            get_customer_booking=lambda **_kwargs: _claim(
        ...
        with (
            ...
            patch.object(
                application_bookings,
                "BookingRepository",
                return_value=repository,
            ),
            self.assertRaisesRegex(BookingNotFound, "уже началась"),
        ):
    ...
    def test_active_customer_booking_keeps_future_appointment(self) -> None:
