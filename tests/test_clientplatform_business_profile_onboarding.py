...
def test_explicit_profile_extraction_never_invents_missing_fields() -> None:
    ...
    assert details.audiences == ("взрослые с тревогой",)
    assert details.contacts == ("@maria_help",)
    assert details.source_urls == ("https://example.test/about",)
    assert details.services == ()
    assert details.legal_constraints == ()
    assert details.preferred_conversion_action is None


def test_profile_details_json_round_trip_is_stable_and_strict() -> None:
    ...
    assert business_profile_details_from_json(encoded) == details
    assert business_profile_details_to_json(business_profile_details_from_json(encoded)) == encoded
    with pytest.raises(ValueError, match="unsupported fields"):
        business_profile_details_from_json('{"secret_memory":"no"}')


def test_review_contains_only_non_empty_owner_facts() -> None:
    lines = business_profile_review_lines(
        BusinessProfileDetails(prices=("2500 ₽",), geo=("Казань",))
    )
    assert lines == ("Цены: 2500 ₽", "География: Казань")


def test_profile_details_are_durable_confirmed_and_tenant_scoped() -> None:
    ...
    try:
        ...
        assert saved_a.confirmed is False
        assert saved_b.confirmed is False
        assert details_repo.get(actor=actor_a).details.prices == ("5000 ₽",)
        assert details_repo.get(actor=actor_b).details.prices == ("9000 ₽",)

        confirmed = details_repo.confirm(
            actor=actor_a,
            now="2026-08-19T00:01:00+00:00",
        )
        assert confirmed.confirmed_at == "2026-08-19T00:01:00+00:00"
        assert details_repo.get(actor=actor_b).confirmed is False
...
def test_schema_additive_migration_keeps_existing_profile_data() -> None:
    conn = sqlite3.connect(":memory:")
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA foreign_keys=ON")
    clientplatform_tenancy.ensure(conn)
    ...
    assert "profile_details_json" in columns
    assert "profile_confirmed_at" in columns
    conn.close()
