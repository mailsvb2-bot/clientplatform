...
from scripts.a1_ephemeral_media_gateway import ephemeral_gateway_config
from scripts.a1_staging_fixture import fixture_bytes, write_fixture
from scripts.a1_telegram_staging_smoke import (
    _extract_start_chat_ids,
    _resolve_chat_id,
)


class A1StagingFixtureTests(unittest.TestCase):
...
class A1EphemeralGatewayConfigTests(unittest.TestCase):
    def test_gateway_is_loopback_filesystem_only(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            with patch.dict(
                os.environ,
                {
                    "A1_MEDIA_GATEWAY_FILESYSTEM_ROOT": temp,
                    "A1_MEDIA_GATEWAY_ALLOWED_BUCKETS": "a1-staging",
        ...
        self.assertEqual(config.allowed_buckets, frozenset({"a1-staging"}))
        self.assertEqual(config.route_prefix, "/a1")
        self.assertFalse(config.s3_endpoint)


class A1TelegramChatDiscoveryTests(unittest.IsolatedAsyncioTestCase):
    def test_extracts_only_private_start_messages(self) -> None:
        updates = [
            ...
            {
                "message": {
                    "text": "/start@a1_staging_bot payload",
                    "chat": {"id": "101", "type": "private"},
                }
            },
            {
    ...
    async def test_active_webhook_fails_closed(self) -> None:
        async def post_json(_url, _payload, _timeout):
            return 200, {
                "ok": True,
                "result": {"url": "https://webhook.example.test/a1"},
            }
...
class A1StagingWorkflowContractTests(unittest.TestCase):
    def test_workflow_has_pinned_ephemeral_tunnel_and_one_required_secret(self) -> None:
        ...
        self.assertNotIn("secrets.A1_MEDIA_SIGNING_KEY", workflow)
        self.assertNotIn("vars.A1_MEDIA_GATEWAY_BASE_URL", workflow)
        self.assertNotIn("vars.A1_STAGING_MEDIA_REFERENCE", workflow)
