...
class CockpitBusinessWorkspaceHttpM7008Tests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_service_create_uses_verified_identity_and_forwards_operation_id(self) -> None:
    ...
    async def test_service_create_rejects_missing_operation_id_before_owner_call(self) -> None:
        principal = TelegramWebAppPrincipal(user_id=303, auth_date=1, query_id=None)
        with (
            patch.object(cockpit_http.settings, "BOT_TOKEN", _TOKEN),
            patch.object(cockpit_http, "verify_telegram_webapp_init_data", return_value=principal),
            patch.object(cockpit_http, "create_cockpit_service") as create,
        ):
        ...
        self.assertEqual(payload["error"], "invalid_services_change")
    ...
    async def test_hidden_growth_route_uses_verified_identity_and_selected_business(self) -> None:
        principal = TelegramWebAppPrincipal(user_id=909, auth_date=1, query_id=None)
        calls: list[dict[str, object]] = []
        ...
        with (
            ...
            patch.object(cockpit_http, "_telegram_action_url", return_value="https://t.me/example?start=route"),
        ):
            status, payload, _headers = await self._post(
        ...
        self.assertEqual(status, 200)
        self.assertEqual(payload["route_url"], "https://t.me/example?start=route")
        self.assertEqual(
            calls,
            [{
