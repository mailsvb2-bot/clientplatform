...
class AdSpendGuardTests(unittest.TestCase):
    def test_guard_allows_only_fresh_matching_authorized_state(self) -> None:
        snapshot = _snapshot()
    ...
    def test_guard_stops_at_server_side_caps(self) -> None:
        cases = (
            (2_000, 100, AdSpendStopReason.DAILY_CAP),
            (10, 10_000, AdSpendStopReason.HARD_CAP),
        )
        for spent_today, total_spent, reason in cases:
    ...
    def test_guard_fails_closed_for_stale_provider_evidence(self) -> None:
        fresh = _snapshot()
        ...
        self.assertFalse(decision.allowed)
        self.assertEqual(decision.stop_reason, AdSpendStopReason.SNAPSHOT_STALE)

    def test_guard_rejects_provider_identity_mismatch(self) -> None:
        original = _snapshot()
    ...
    def test_grant_requires_exact_terms_and_snapshot_hash(self) -> None:
        snapshot = _snapshot()
