...
from clientplatform.infrastructure import dispatch_observability
...
class DispatchOutboxObservabilityTests(unittest.TestCase):
    def test_aggregate_snapshot_is_global_count_only_and_time_bounded(self) -> None:
        ...
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_pending'], 2)
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_retry'], 3)
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_due'], 5)
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_stale_sending'], 1)
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_recent_dead'], 2)
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_oldest_due_age_seconds'], 120)
    ...
    def test_invalid_oldest_timestamp_does_not_break_health(self) -> None:
        ...
        self.assertEqual(snapshot['clientplatform_dispatch_outbox_oldest_due_age_seconds'], 0)
    ...
    def test_missing_aggregate_row_fails_closed(self) -> None:
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(
                RuntimeError,
                'clientplatform_dispatch_outbox_aggregate_unavailable',
            ):
