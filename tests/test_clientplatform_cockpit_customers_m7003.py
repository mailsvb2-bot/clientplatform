...
class CockpitCustomersM7003Tests(unittest.TestCase):
    ...
    def test_mobile_asset_is_read_only_and_keeps_authority_on_server(self) -> None:
        ...
        self.assertIn("payload = await hydrateActionRoute(payload)", script)
        self.assertIn("openTelegramRoute(routeUrl)", script)
        self.assertNotIn("const openActionRoute = async", script)
