...
from a1.runtime import lifecycle, owner
...
class A1SchemaReadinessTests(unittest.TestCase):
    def test_complete_a1_schema_is_required(self) -> None:
        complete = sorted(owner._A1_REQUIRED_TABLES)
        with (
            ...
            self.assertEqual(owner._a1_schema_readiness(), (True, None))

        with (
            patch.object(owner, "CONFIG", SimpleNamespace(uses_postgres=False)),
            patch.object(owner, "get_connection", return_value=_ConnectionContext([(name,) for name in complete[:-1]])),
        ):
            ready, error = owner._a1_schema_readiness()
        self.assertFalse(ready)
        self.assertEqual(error, f"a1_schema_missing:{complete[-1]}")


class A1RuntimeOwnerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_disabled_owner_is_completely_dormant(self) -> None:
        ...
        await owner.run_a1_runtime_owner(
    ...
    async def test_owner_fails_closed_when_schema_never_becomes_ready(self) -> None:
        ...
        with (
            ...
            with self.assertRaisesRegex(RuntimeError, "a1_runtime_schema_timeout"):
                await owner.run_a1_runtime_owner(
                    config=_config(enabled=True),
                    schema_probe=lambda: (False, "a1_schema_missing:connections"),
