...
class ScriptedActions(YandexDirectAdActions):
    def __init__(self, responses: list[Mapping[str, Any]]) -> None:
        super().__init__(oauth=YandexOAuthConfig(client_id="id", client_secret="secret", redirect_uri="https://example.test/callback"))
...
class SpendOperationDomainTests(unittest.TestCase):
    def test_key_is_tenant_and_operation_scoped(self) -> None:
        ...
        launch = ad_spend_operation_key(business_id=business, authorization_id=authorization, operation_type="launch")
        self.assertEqual(
            launch,
            ad_spend_operation_key(business_id=business, authorization_id=authorization, operation_type=AdSpendOperationType.LAUNCH),
        )
        self.assertNotEqual(
            launch,
            ad_spend_operation_key(business_id=business, authorization_id=authorization, operation_type="stop"),
        )
    ...
    def test_processing_requires_a_lease(self) -> None:
        ...
        with self.assertRaisesRegex(AdSpendInvariantViolation, "requires a lease"):
            AdSpendOperation(
                ...
                idempotency_key=ad_spend_operation_key(business_id=business, authorization_id=authorization, operation_type="launch"),
                attempts=1,
    ...
    def test_provider_launch_reconciles_submitted_ad_without_second_mutation(self) -> None:
        provider = ScriptedActions([
            {"Ads": [{"Id": 77, "AdGroupId": 88, "CampaignId": 99, "State": "ON", "Status": "MODERATION", "Type": "TEXT_AD"}]}
        ])
    ...
    def test_provider_launch_mutates_only_exact_draft_and_rechecks(self) -> None:
        provider = ScriptedActions([
            {"Ads": [{"Id": 77, "AdGroupId": 88, "CampaignId": 99, "State": "OFF", "Status": "DRAFT", "Type": "TEXT_AD"}]},
            {"ModerateResults": [{"Id": 77}]},
            {"Ads": [{"Id": 77, "AdGroupId": 88, "CampaignId": 99, "State": "OFF", "Status": "MODERATION", "Type": "TEXT_AD"}]},
        ])
        result = provider.moderate_ad(
            access_token="token",
            external_ad_id="77",
    ...
    def test_provider_rejects_cross_campaign_identity(self) -> None:
        provider = ScriptedActions([
            {"Ads": [{"Id": 77, "AdGroupId": 88, "CampaignId": 100, "State": "OFF", "Status": "DRAFT", "Type": "TEXT_AD"}]}
        ])
        with self.assertRaisesRegex(YandexDirectError, "campaign_identity_mismatch"):
    ...
    def test_stop_is_idempotent_for_already_suspended_ad(self) -> None:
        provider = ScriptedActions([
            {"Ads": [{"Id": 77, "AdGroupId": 88, "CampaignId": 99, "State": "SUSPENDED", "Status": "ACCEPTED", "Type": "TEXT_AD"}]}
        ])
