...
from a1.domain.programs import ContentKind, DeliveryStatus, ProgressStatus
from a1.domain.tenancy import PlatformRole, TenantPermissionDenied
...
class A1DispatchFixture:
    def __init__(self) -> None:
        ...
        self.lesson_a = self.programs.add_lesson(
            ...
            title="Первое аудио",
            content_kind="audio",
            content_ref="s3://a1/sleep-01.mp3",
        )
        ...
        self.connection_a = self.connections.create_connection(
            ...
            external_account_id="a1-shared-bot",
            credential_reference="secret://a1/business-a/telegram-token",
...
class A1ConnectionsDispatchOutboxTests(unittest.TestCase):
    def setUp(self) -> None:
        self.fx = A1DispatchFixture()
    ...
    def test_raw_tokens_and_platform_type_mismatch_are_rejected(self) -> None:
        ...
        with self.assertRaises(ConnectionInvariantViolation):
            self.fx.connections.create_connection(
                ...
                credential_reference="secret://a1/wrong-platform",
            )
    ...
    def test_only_owner_or_administrator_manages_connections(self) -> None:
        ...
        with self.assertRaises(TenantPermissionDenied):
            self.fx.connections.create_connection(
                ...
                credential_reference="secret://a1/support-bot",
            )
    ...
    def test_managed_bot_requires_managed_connection_and_secret_reference(self) -> None:
        with self.assertRaises(ConnectionNotFound):
            self.fx.connections.register_managed_bot(
                ...
                external_bot_id="111",
                webhook_secret_reference="secret://a1/webhook/111",
            )
        managed_connection = self.fx.connections.create_connection(
            ...
            credential_reference="vault://a1/managed/111/token",
        )
        with self.assertRaises(ConnectionInvariantViolation):
            self.fx.connections.register_managed_bot(
                actor=self.fx.owner_a,
        ...
        bot = self.fx.connections.register_managed_bot(
            ...
            connection_id=managed_connection.id,
            external_bot_id="111",
            webhook_secret_reference="secret://a1/managed/111/webhook",
    ...
    def test_database_rejects_cross_platform_dispatch_link(self) -> None:
        ...
        with self.assertRaises(sqlite3.IntegrityError):
            self.fx.conn.execute(
                ...
                ) VALUES(?,?,'telegram',?,?,?,'audio','s3://a1/x.mp3',?,
                         'pending',0,?,?,?)
                """,
                (
    ...
    def test_privacy_manifest_covers_connection_and_outbox_tables(self) -> None:
        report = validate_a1_privacy_manifest(self.fx.conn, strict=True)
...
class A1DispatchWorkerTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self) -> None:
        self.fx = A1DispatchFixture()
    ...
    async def test_worker_resolves_secret_outside_payload_and_marks_sent(self) -> None:
        ...
        self.assertEqual(
            ...
            ["secret://a1/business-a/telegram-token"],
        )
        dump = "\n".join(
            str(tuple(row))
