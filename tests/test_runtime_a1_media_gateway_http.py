...
from a1.transport.media import HmacMediaGatewayResolver, media_gateway_signature
...
class A1MediaGatewayHttpTests(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        target = self.root / "private-bucket" / "program" / "audio.mp3"
        target.parent.mkdir(parents=True)
        target.write_bytes(b"A1-AUDIO-CONTENT")
        self.secret = "media-gateway-test-secret"
        self.provider = EnvironmentCredentialProvider(
            {
                "A1_SECRET_MEDIA_SIGNING_KEY": self.secret,
                "A1_SECRET_TELEGRAM_MAIN": "telegram-test-token",
            }
        )
    ...
    async def test_signed_gateway_streams_full_head_and_range_responses(self) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self._local_url(signed)) as response:
                ...
                self.assertEqual(await response.read(), b"A1-AUDIO-CONTENT")
    ...
    async def test_bad_expired_and_disallowed_requests_fail_closed(self) -> None:
        ...
        expired_path = "/a1/media/private-bucket/program/audio.mp3"
    ...
    async def test_telegram_adapter_resolves_gateway_and_fake_provider_fetches_media(self) -> None:
        ...
        self.assertEqual(fetched, [b"A1-AUDIO-CONTENT"])
        self.assertEqual(provider_payloads[0]["chat_id"], "700001")
        self.assertTrue(provider_payloads[0]["audio"].startswith("https://media.example.test/a1/media/"))
