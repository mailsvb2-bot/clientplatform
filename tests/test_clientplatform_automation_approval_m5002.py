...
def test_money_approval_currency_must_be_known_settlement_iso4217() -> None:
    for currency in ("XXX", "ZZZ"):
        with pytest.raises(ValueError, match="known settlement ISO 4217"):
            AutomationMoneyLimit(
...
def test_candidate_fingerprint_round_trip_is_canonical(fx: Fixture) -> None:
...
def test_request_is_durable_and_exact_replay_is_idempotent(fx: Fixture) -> None:
    first = fx.request()
    replay = fx.request()
...
def test_same_idempotency_key_with_changed_candidate_fails_closed(fx: Fixture) -> None:
    fx.request()
    with pytest.raises(AutomationApprovalConflict, match="idempotency_conflict"):
...
def test_allow_or_denied_action_cannot_fabricate_an_approval(fx: Fixture) -> None:
    ...
    with pytest.raises(AutomationApprovalConflict, match="approval_not_required"):
        fx.repo.request_action_approval(
            actor=fx.owner,
            candidate=allowed,
    ...
    with pytest.raises(AutomationApprovalConflict, match="denied_by_policy"):
        fx.repo.request_action_approval(
            actor=fx.owner,
            candidate=denied,
...
def test_business_automation_roles_can_read_but_only_owner_can_decide(fx: Fixture) -> None:
    ...
    with pytest.raises(TenantPermissionDenied, match="owner"):
    ...
    with pytest.raises(TenantPermissionDenied, match="not allowed"):
        fx.repo.list_pending_action_approvals(actor=fx.members[PlatformRole.ANALYST], now=_NOW)


def test_owner_approval_is_idempotent_and_authorization_is_deterministic(fx: Fixture) -> None:
    approval = fx.request()
...
def test_reject_is_idempotent_and_cannot_be_approved_afterward(fx: Fixture) -> None:
    approval = fx.request()
    ...
    with pytest.raises(AutomationApprovalConflict, match="already_decided"):
        fx.repo.approve_action_approval(
            actor=fx.owner,
            approval_id=approval.id,
            expected_request_fingerprint=approval.request_fingerprint,
...
def test_approved_action_can_be_revoked_and_no_longer_authorizes(fx: Fixture) -> None:
    approval = fx.request()
    approved = fx.repo.approve_action_approval(
        actor=fx.owner,
        approval_id=approval.id,
    ...
    with pytest.raises(AutomationApprovalConflict, match="not_approved"):
...
def test_policy_change_invalidates_pending_and_approved_authority(fx: Fixture) -> None:
    ...
    with pytest.raises(AutomationApprovalConflict, match="policy_changed"):
        fx.repo.approve_action_approval(
            actor=fx.owner,
...
def test_new_policy_after_action_approval_invalidates_authorization(fx: Fixture) -> None:
...
def test_expiry_and_expected_hashes_fail_closed(fx: Fixture) -> None:
    ...
    assert fx.repo.list_pending_action_approvals(actor=fx.owner, now=_NOW + timedelta(minutes=3)) == ()
    with pytest.raises(AutomationApprovalConflict, match="changed"):
        fx.repo.approve_action_approval(
            actor=fx.owner,
            approval_id=approval.id,
    ...
    with pytest.raises(AutomationApprovalConflict, match="expired"):
        fx.repo.approve_action_approval(
            actor=fx.owner,
            approval_id=approval.id,
            expected_request_fingerprint=approval.request_fingerprint,
...
def test_cross_tenant_approval_lookup_fails_closed(fx: Fixture) -> None:
    approval = fx.request()
    with pytest.raises(AutomationApprovalNotFound):
        fx.repo.get_action_approval(actor=fx.other_owner, approval_id=approval.id)


def test_authorization_requires_exact_candidate_hash(fx: Fixture) -> None:
    approval = fx.request()
    approved = fx.repo.approve_action_approval(
        actor=fx.owner,
        approval_id=approval.id,
    ...
    with pytest.raises(AutomationApprovalConflict, match="candidate_changed"):
        fx.repo.get_action_authorization(
            actor=fx.owner,
...
def test_schema_privacy_and_owner_copy_cover_action_approval(fx: Fixture) -> None:
