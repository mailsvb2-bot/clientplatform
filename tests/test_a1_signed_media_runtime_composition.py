...
from a1.application.dispatch_worker import _effective_max_attempts
...
from a1.runtime import lifecycle
from a1.runtime.dispatch_runtime import DispatchRuntimeConfig
from a1.runtime.scheduler import DispatchSchedulerHealth
...
from a1.transport.telegram import TelegramDispatchAdapter
from a1.transport.telegram_http import TelegramBotApiError
...
class SignedMediaResolverTests(unittest.IsolatedAsyncioTestCase):
    async def test_s3_reference_becomes_deterministic_short_lived_https_url(self) -> None:
        ...
        resolver = HmacMediaGatewayResolver(
            base_url="https://media.example.test/a1",
            credential_provider=EnvironmentCredentialProvider(
                {"A1_SECRET_MEDIA_SIGNING_KEY": secret}
            ),
        ...
        self.assertIn("/a1/media/private-bucket/programs/", parsed.path)
...
class RuntimeLifecycleTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_lifecycle_is_explicit_single_owner_and_observable(self) -> None:
        ...
        with patch.object(lifecycle, "A1DispatchScheduler", FakeScheduler):
            self.assertTrue(await lifecycle.start_a1_runtime(runtime))
            self.assertFalse(await lifecycle.start_a1_runtime(runtime))
            snapshot = lifecycle.a1_runtime_health_snapshot()
            self.assertTrue(snapshot["a1_runtime_composed"])
            self.assertTrue(snapshot["a1_dispatch_running"])
            self.assertEqual(snapshot["a1_dispatch_sent"], 2)
            await lifecycle.stop_a1_runtime()
        self.assertTrue(FakeScheduler.instances[0].stopped)
        self.assertFalse(lifecycle.a1_runtime_health_snapshot()["a1_runtime_composed"])
