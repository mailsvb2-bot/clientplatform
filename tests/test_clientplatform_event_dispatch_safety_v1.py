...
def test_superseded_followup_revision_is_cancelled_before_provider_write() -> None:
    conn = _db()
    conn.execute(
        """
        CREATE TABLE clientplatform_event_content_messages(
    ...
    key = "event:e:registration:r:message:post:v5:stage:1:slot:no_show-1:revision:1"
    conn.execute(
        "UPDATE provider_dispatch_outbox SET idempotency_key=? WHERE id='d'",
        (key,),
    )
    ...
    assert tuple(row) == ("cancelled", "event_followup_revision_superseded")


def test_current_followup_revision_can_cross_when_other_authority_is_valid() -> None:
    conn = _db()
    conn.execute(
        """
        CREATE TABLE clientplatform_event_content_messages(
