...
from a1.application.dispatch_worker import run_dispatch_batch
from a1.domain.programs import DeliveryStatus, ProgressStatus
from a1.infrastructure import (
    ConnectionRepository,
    DispatchOutboxRepository,
    TenancyRepository,
)
from a1.infrastructure.customer_repository import CustomerRepository
from a1.infrastructure.delivery_repository import DeliveryRepository
from a1.infrastructure.program_repository import ProgramRepository
from a1.runtime.media_gateway import (
...
from a1.transport.media import HmacMediaGatewayResolver
...
class A1PersistedMediaDeliveryRestartE2E(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        self.db_path = self.root / "a1-staging.sqlite3"
        self.media_root = self.root / "objects"
        target = self.media_root / "private-bucket" / "programs" / "lesson-01.mp3"
        target.parent.mkdir(parents=True)
        target.write_bytes(b"PERSISTED-A1-AUDIO")
        self.provider = EnvironmentCredentialProvider(
            {
                "A1_SECRET_TELEGRAM_MAIN": "telegram-restart-token",
                "A1_SECRET_MEDIA_SIGNING_KEY": "restart-signing-secret",
            }
        )
        ...
        self.gateway_config = MediaGatewayConfig(
            ...
            host="127.0.0.1",
            port=0,
            public_base_url="https://media.example.test/a1",
            storage_mode="filesystem",
            allowed_buckets=frozenset({"private-bucket"}),
            filesystem_root=str(self.media_root),
            s3_endpoint="",
            s3_region="",
            s3_access_key_reference="secret://env/A1_SECRET_S3_ACCESS_KEY",
            s3_secret_key_reference="secret://env/A1_SECRET_S3_SECRET_KEY",
            s3_session_token_reference="",
            signing_secret_reference="secret://env/A1_SECRET_MEDIA_SIGNING_KEY",
    ...
    def _seed_database(self) -> str:
        conn = self._connect()
        try:
            a1_tenancy.ensure(conn)
            a1_customers.ensure(conn)
            a1_programs.ensure(conn)
            a1_connections.ensure(conn)
            ...
            connection = connections.create_connection(
                ...
                external_account_id="a1-staging-bot",
                credential_reference="secret://env/A1_SECRET_TELEGRAM_MAIN",
    ...
    async def test_delivery_persists_across_restart_and_is_not_resent(self) -> None:
        with patch(
            "a1.application.dispatch_worker.get_db",
        ...
        self.assertEqual(self.provider_media, [b"PERSISTED-A1-AUDIO"])
