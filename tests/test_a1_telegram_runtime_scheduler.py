...
from a1.application.dispatch_worker import DispatchBatchResult
...
from a1.transport import AdapterRegistry
...
def _runtime(*, enabled: bool, tick_timeout_seconds: float = 0.05) -> DispatchRuntime:
    return DispatchRuntime(
        ...
        credential_provider=EnvironmentCredentialProvider(
            {"A1_SECRET_TELEGRAM_MAIN": "123456:TEST_TOKEN"}
        ),
        adapters=AdapterRegistry([]),
    )


class A1SecretProviderTests(unittest.TestCase):
    def test_environment_reference_resolves_without_exposing_value(self) -> None:
        secret = "123456:VERY_PRIVATE_TOKEN"
        provider = EnvironmentCredentialProvider(
            {"A1_SECRET_TELEGRAM_MAIN": secret}
        )
        self.assertEqual(
            provider.resolve("secret://env/A1_SECRET_TELEGRAM_MAIN"),
            secret,
        )
    ...
    def test_runtime_is_disabled_by_default_and_bounds_environment(self) -> None:
        ...
        with patch.dict(
            "os.environ",
            {
                "A1_DISPATCH_RUNTIME_ENABLED": "yes",
                "A1_DISPATCH_BATCH_SIZE": "10000",
                "A1_DISPATCH_TICK_TIMEOUT_SEC": "1",
            },
            clear=True,
        ):
            config = dispatch_runtime_config()
...
class A1TelegramHttpClientTests(unittest.IsolatedAsyncioTestCase):
...
class A1DispatchRuntimeTests(unittest.IsolatedAsyncioTestCase):
