...
class A1ProgramDeliveryVerticalTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for position in range(1, lessons + 1):
            created_lessons.append(
                self.programs.add_lesson(
                    ...
                    content_kind="audio",
                    content_ref=f"s3://a1/lesson-{position}.mp3",
                )
            )
    ...
    def test_privacy_manifest_covers_complete_vertical(self) -> None:
        report = validate_a1_privacy_manifest(self.conn, strict=True)
