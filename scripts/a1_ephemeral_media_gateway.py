...
from a1.transport.media import parse_s3_reference
...
def ephemeral_gateway_config() -> MediaGatewayConfig:
    filesystem_root = str(
        Path(_required_env("A1_MEDIA_GATEWAY_FILESYSTEM_ROOT"))
        .expanduser()
        .resolve()
    )
    bucket, _ = parse_s3_reference(
        f"s3://{_required_env('A1_MEDIA_GATEWAY_ALLOWED_BUCKETS')}/fixture.mp3"
    )
    return MediaGatewayConfig(
        ...
        public_base_url=str(
            os.getenv("A1_STAGING_GATEWAY_ROUTE_BASE_URL")
            or "https://staging.invalid/a1"
        ).strip(),
...
def main() -> int:
    try:
        ...
        print(f"A1 ephemeral gateway failed: {exc}", flush=True)
        return 1
    except ValueError as exc:
        print(f"A1 ephemeral gateway failed: {type(exc).__name__}", flush=True)
        return 1
    return 0
