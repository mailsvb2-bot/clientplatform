...
from clientplatform.transport.media import HmacMediaGatewayResolver, MediaReferenceError
...
async def _resolve_chat_id(
    *,
    ...
    timeout_seconds = env_float(
        "CLIENTPLATFORM_STAGING_TELEGRAM_DISCOVERY_TIMEOUT_SEC",
        20.0,
        minimum=1.0,
        maximum=120.0,
    )
...
async def _probe_gateway(url: str) -> None:
    try:
        ...
        raise RuntimeError("staging_http_dependency_missing") from None
    timeout_seconds = env_float(
        "CLIENTPLATFORM_STAGING_GATEWAY_TIMEOUT_SEC",
        20.0,
        minimum=1.0,
        maximum=120.0,
    )
...
async def run() -> None:
    ...
    token_reference = str(
        os.getenv("CLIENTPLATFORM_STAGING_TELEGRAM_TOKEN_REFERENCE")
        or "secret://env/CLIENTPLATFORM_SECRET_STAGING_TELEGRAM_BOT_TOKEN"
    ).strip()
    ...
    media_reference = _required_env("CLIENTPLATFORM_STAGING_MEDIA_REFERENCE")
    gateway_base_url = _required_env("CLIENTPLATFORM_MEDIA_GATEWAY_BASE_URL")
    telegram_base_url = str(
        os.getenv("CLIENTPLATFORM_TELEGRAM_API_BASE_URL") or "https://api.telegram.org"
    ).strip()
    chat_id = await _resolve_chat_id(
        token=token,
        telegram_base_url=telegram_base_url,
        explicit_chat_id=str(os.getenv("CLIENTPLATFORM_STAGING_TELEGRAM_CHAT_ID") or ""),
    )
    ...
    print(f"clientplatform Telegram staging smoke passed; message_id={message_id}")
...
def main() -> int:
    try:
        ...
        print(f"clientplatform Telegram staging smoke failed: {type(exc).__name__}")
        return 1
    except MediaReferenceError as exc:
        print(f"clientplatform Telegram staging smoke failed: {type(exc).__name__}")
        return 1
    except TelegramBotApiError as exc:
        print(f"clientplatform Telegram staging smoke failed: {exc.code}")
        return 1
    except RuntimeError as exc:
        print(f"clientplatform Telegram staging smoke failed: {exc}")
        return 1
    return 0
