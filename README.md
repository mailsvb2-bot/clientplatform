...
### AI Commerce и умный контроль тарифа

Владелец получает контекстный commerce-слой поверх канонического subscription state, а не отдельный billing ledger:

- фактическое использование сотрудников и клиентов читается из tenant-scoped business data;
- текущие allowance берутся из существующего состояния тарифа;
- ClientPlatform заранее показывает, входит ли следующий сотрудник или клиент в текущий лимит;
- добавление нового сотрудника не начинается молча, если seat-limit уже исчерпан;
- входящий клиент, регистрация или лид не теряется только из-за превышения customer allowance — владелец получает понятный сигнал о необходимости расширения;
- запрос на расширение создаёт idempotent billing support case;
- proposal сам по себе не меняет тариф, не создаёт оплату, не выдаёт entitlement и не выполняет provider mutation.

`operation_id` остаётся границей одной бизнес-операции, а authoritative outcomes продолжают храниться только в существующем `BusinessOutcomeEvent` / `OutcomeRepository`.
