...
class TenancyRepository:
    ...
    def create_business(
        ...
        *,
        ...
        new_business_id = normalize_uuid(business_id or str(uuid4()), field_name="business_id")
    ...
    def list_accessible_businesses(self, *, user_id: int) -> list[BusinessAccess]:
        ...
        for row in rows:
            ...
            accesses.append(BusinessAccess(business=business, membership=membership))
        return accesses

    def rename_business(self, *, actor: TenantContext, name: str, now: str | None = None) -> Business:
        ...
        timestamp = str(now or _utc_now())
        self._conn.execute(
            "UPDATE businesses SET name=?, updated_at=? WHERE id=? AND status='active'",
            (normalized_name, timestamp, current_actor.business_id),
        )
        return self.get_access(user_id=current_actor.user_id, business_id=current_actor.business_id).business
        ...
        *,
        ...
        if existing is not None:
            ...
            if current_actor.role == PlatformRole.ADMINISTRATOR and existing_role not in {
                ...
                raise TenantPermissionDenied("administrator cannot modify owner or administrator membership")
            ...
            self._conn.execute(
                ...
                """,
                (target_role.value, timestamp, membership_id, current_actor.business_id),
            )
        else:
            membership_id = str(uuid4())
            self._conn.execute(
        ...
        return self._get_member(membership_id=membership_id, business_id=current_actor.business_id)
    ...
    def revoke_member(
        ...
        *,
        ...
        if target_role == PlatformRole.OWNER:
            ...
            if owner_count <= 1:
                raise TenantInvariantViolation("a business must retain at least one active owner")
        ...
        self._conn.execute(
            ...
            WHERE id=? AND business_id=? AND status='active'
            """,
            (timestamp, timestamp, membership_id, current_actor.business_id),
        )
        return self._get_member(membership_id=membership_id, business_id=current_actor.business_id)

    def _get_member(self, *, membership_id: str, business_id: str) -> BusinessMember:
        normalized_membership_id = normalize_uuid(membership_id, field_name="membership_id")
        normalized_business_id = normalize_uuid(business_id, field_name="business_id")
        row = self._conn.execute(
            """
            SELECT id, business_id, user_id, role, status, created_at, updated_at, revoked_at
        ...
        if row is None:
            raise TenantAccessDenied("membership was not found in the active business scope")
