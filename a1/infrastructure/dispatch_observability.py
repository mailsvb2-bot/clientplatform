...
def load_dispatch_outbox_snapshot(
    *,
    ...
    """Return aggregate A1 dispatch pressure without exposing tenant or payload data."""
    ...
    if row is None:
        raise RuntimeError('a1_dispatch_outbox_aggregate_unavailable')
    ...
    return {
        ...
        'a1_dispatch_outbox_pending': _count(row, 'pending_count', 0),
        'a1_dispatch_outbox_retry': _count(row, 'retry_count', 1),
        'a1_dispatch_outbox_sending': _count(row, 'sending_count', 2),
        'a1_dispatch_outbox_sent': _count(row, 'sent_count', 3),
        'a1_dispatch_outbox_dead': _count(row, 'dead_count', 4),
        'a1_dispatch_outbox_cancelled': _count(row, 'cancelled_count', 5),
        'a1_dispatch_outbox_due': _count(row, 'due_count', 6),
        'a1_dispatch_outbox_stale_sending': _count(row, 'stale_sending_count', 7),
        'a1_dispatch_outbox_recent_dead': _count(row, 'recent_dead_count', 8),
        'a1_dispatch_outbox_oldest_due_age_seconds': oldest_due_age_seconds,
