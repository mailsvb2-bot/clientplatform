...
NAME = "clientplatform_provider_dispatch_interactions_v1"
...
def _rebuild_sqlite(conn: sqlite3.Connection) -> None:
    ...
    if "customer_interaction" in sql:
        return
    ...
    conn.execute("DROP TABLE IF EXISTS provider_dispatch_outbox_interactions_v0")
    conn.execute(
        "ALTER TABLE provider_dispatch_outbox RENAME TO provider_dispatch_outbox_interactions_v0"
    )
    clientplatform_provider_dispatch.ensure(conn)
    conn.execute(
        """
        ...
        SELECT
            ...
            attempts,available_at,locked_at,lock_token,provider_message_id,
            last_error,created_at,updated_at,sent_at,dead_at
        FROM provider_dispatch_outbox_interactions_v0
        """
    )
    conn.execute("DROP TABLE provider_dispatch_outbox_interactions_v0")
...
def _update_postgres(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(
        ...
        ADD CONSTRAINT cp_provider_dispatch_source_kind_interactions_v1
    ...
    conn.execute(
        """
        ALTER TABLE provider_dispatch_outbox
        ADD CONSTRAINT cp_provider_dispatch_source_shape_interactions_v1
        CHECK(
