...
starts the small flow again.  Reads never consume state.  A state is removed
only by ``consume_pending`` after the business input has been validated, or by
an explicit cancellation.  This prevents invalid input from silently ending a
flow and lets router filters claim only the state they own.
"""
...
def consume_pending(
    ...
    A mismatching state is left untouched.  This is the primitive handlers use
    after successful validation and cancellation filters use to avoid stealing
